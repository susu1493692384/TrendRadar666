...
def load_config():
    ...
    config = {
        ...
        "WPS_BATCH_SIZE": config_data["notification"].get("wps_batch_size", 5000),
    ...
    config["WPS_WEBHOOK_URL"] = os.environ.get(
        "WPS_WEBHOOK_URL", ""
    ).strip() or webhooks.get("wps_url", "")
    ...
    if config["WPS_WEBHOOK_URL"]:
        source = "环境变量" if os.environ.get("WPS_WEBHOOK_URL") else "配置文件"
        notification_sources.append(f"WPS({source})")
...
def render_wps_content(
    report_data: Dict, update_info: Optional[Dict] = None, mode: str = "daily"
) -> Dict:
    """渲染WPS内容，返回包含title和text的字典"""
    text_content = ""
    title = "热点分析报告"
...
def split_content_into_batches(
    ...
    if max_bytes is None:
        if format_type == "dingtalk":
            ...
            max_bytes = CONFIG.get("WPS_BATCH_SIZE", 20000)
        else:
...
def send_to_notifications(
    ...
    wps_url = CONFIG["WPS_WEBHOOK_URL"]
    ...
    # 发送到 WPS
    if wps_url:
        results["wps"] = send_to_wps(
            wps_url, report_data, report_type, update_info_to_send, proxy_url, mode
        )
...
def send_to_wps(
    ...
    for i, batch_content in enumerate(batches, 1):
        ...
        try:
            ...
            print(f"WPS第 {i}/{len(batches)} 批次发送出错 [{report_type}]：{e}")
            return False

    print(f"WPS所有 {len(batches)} 批次发送完成 [{report_type}]")
    return True
